...
전처리 및 청킹 모듈의 최종 출력은 임베딩 추출 기능의 입력으로 사용될 텍스트 청크들의 리스트입니다. 각 청크는 원본 콘텐츠의 `content_code`와 메타데이터를 함께 포함해야 합니다.
...
5.  **데이터 수집 스크립트(`collect_youtube_data.py`)와 연동**:
    ...
    -   청킹된 결과는 다음 단계인 임베딩 추출 기능으로 전달될 준비가 됩니다.

### 5.1. 전처리된 데이터의 처리 방식 (How Processed Data is Handled)

전처리 및 청킹을 거친 텍스트 데이터(청크)는 별도의 영구 저장소에 저장되지 않습니다. 대신, 다음과 같은 흐름으로 처리됩니다.

1.  **다음 단계로 전달**: 각 텍스트 청크는 임베딩 추출 기능으로 전달되어 벡터로 변환될 준비가 됩니다.
2.  **벡터 DB 저장**: 생성된 임베딩 벡터는 해당 청크의 `content_code`, `chunk_id` 및 필요한 메타데이터와 함께 벡터 데이터베이스(ChromaDB)에 저장됩니다.
3.  **원본 데이터 참조**: 벡터 검색 시 반환되는 `content_code`를 통해 SQLite에 저장된 원본 데이터를 다시 조회하여 활용합니다.
